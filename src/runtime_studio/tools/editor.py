...
from runtime_studio.tools import DEFER_META
...
def register_editor_tools(mcp: FastMCP, *, include_non_core: bool = True) -> None:
    ...
    async def logs_read(
        ...
        - "editor": editor-process script errors — parse errors, @tool/
          ...
          addons/runtime_studio/ dropped. Errors fired before plugin enable are
          not captured.
    ...
    async def editor_reload_plugin(ctx: Context, session_id: str = "") -> dict:
        ...
        - **Externally launched** (e.g. ``python -m runtime_studio --transport
