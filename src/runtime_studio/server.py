"""FastMCP server — the main entry point for Runtime Studio for Godot."""
...
from runtime_studio.asgi import StaleMcpSessionDiagnosticMiddleware
from runtime_studio.godot_client.client import GodotClient
from runtime_studio.middleware import (
    HintOpTypoOnManage,
    ParseStringifiedParams,
    PreserveGodotCommandErrorData,
    StripClientWrapperKwargs,
)
from runtime_studio.resources.editor import register_editor_resources
from runtime_studio.resources.library import register_library_resources
from runtime_studio.resources.nodes import register_node_resources
from runtime_studio.resources.project import register_project_resources
from runtime_studio.resources.scenes import register_scene_resources
from runtime_studio.resources.scripts import register_script_resources
from runtime_studio.resources.sessions import register_session_resources
from runtime_studio.sessions.registry import SessionRegistry
from runtime_studio.telemetry import (
    MilestoneType,
    RecordType,
...
from runtime_studio.tools.animation import register_animation_tools
from runtime_studio.tools.audio import register_audio_tools
from runtime_studio.tools.autoload import register_autoload_tools
from runtime_studio.tools.batch import register_batch_tools
from runtime_studio.tools.camera import register_camera_tools
from runtime_studio.tools.client import register_client_tools
from runtime_studio.tools.editor import register_editor_tools
from runtime_studio.tools.filesystem import register_filesystem_tools
from runtime_studio.tools.game import register_game_tools
from runtime_studio.tools.input_map import register_input_map_tools
from runtime_studio.tools.material import register_material_tools
from runtime_studio.tools.node import register_node_tools
from runtime_studio.tools.particle import register_particle_tools
from runtime_studio.tools.project import register_project_tools
from runtime_studio.tools.resource import register_resource_tools
from runtime_studio.tools.scene import register_scene_tools
from runtime_studio.tools.script import register_script_tools
from runtime_studio.tools.session import register_session_tools
from runtime_studio.tools.signal import register_signal_tools
from runtime_studio.tools.testing import register_testing_tools
from runtime_studio.tools.theme import register_theme_tools
from runtime_studio.tools.ui import register_ui_tools
...
## Filesystem location of the running `runtime_studio` package — surfaced via the
## /runtime-studio-godot/status probe so the editor's "Incompatible server" diagnostic
## can tell the user *which* `src/runtime_studio/` was actually loaded. In a
...
_SERVER_PACKAGE_PATH = str(Path(_runtime_studio_pkg.__file__).resolve().parent)
...
class GodotAIFastMCP(FastMCP):
    """FastMCP server with Runtime Studio for Godot's ASGI diagnostics for HTTP transports."""
    ...
    def http_app(self, *args: Any, **kwargs: Any):
        ...
        ## including ``sse`` so ``/runtime-studio-godot/status`` and the FastMCP
...
def create_server(
    ...
    *,
    ...
    @mcp.custom_route("/runtime-studio-godot/status", methods=["GET"], include_in_schema=False)
    async def runtime_studio_status(_request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            {
                ...
                ## "loaded from /Users/.../runtime-studio-godot-feature-branch/src" vs
                ## "loaded from /Users/.../runtime-studio-godot/src") without the
