"""Anonymous, privacy-focused telemetry for Runtime Studio for Godot.
...
* No pyproject.toml walk — ``runtime_studio.__version__`` is canonical.
* Single opt-out env (``RUNTIME_STUDIO_DISABLE_TELEMETRY``) plus the shared
  ``DISABLE_TELEMETRY``.
* Session-id slugs are hashed before leaving the process (privacy: project
  ...
  verbatim).
* Endpoint is opt-in via ``RUNTIME_STUDIO_TELEMETRY_ENDPOINT``. Empty default
...
from runtime_studio import __version__ as _PACKAGE_VERSION
from runtime_studio.protocol.errors import ErrorCode

logger = logging.getLogger("runtime-studio-godot-telemetry")
...
class TelemetryConfig:
    ...
    ``RUNTIME_STUDIO_DISABLE_TELEMETRY=true`` (or the cross-tool
    ``DISABLE_TELEMETRY=true``). ``RUNTIME_STUDIO_TELEMETRY_ENDPOINT``
    ...
    ## setting ``RUNTIME_STUDIO_TELEMETRY_ENDPOINT`` (e.g. self-hosters, the
    ...
    DEFAULT_ENDPOINT = "https://runtime-studio-godot-telemetry-pudmurzsnq-uw.a.run.app/events"
    DEFAULT_TIMEOUT = 1.5
    ...
    def __init__(self) -> None:
        ...
        self.allow_loopback = self._env_truthy("RUNTIME_STUDIO_TELEMETRY_ALLOW_LOOPBACK")
    ...
    def _is_disabled_via_env(cls) -> bool:
        return cls._env_truthy("RUNTIME_STUDIO_DISABLE_TELEMETRY") or cls._env_truthy(
            "DISABLE_TELEMETRY"
        )
    ...
    def _resolve_endpoint(self) -> str:
        ...
        ## the wrong place.)
        raw = os.environ.get("RUNTIME_STUDIO_TELEMETRY_ENDPOINT", "").strip()
        if not raw:
    ...
    def _resolve_timeout(self) -> float:
        raw = os.environ.get("RUNTIME_STUDIO_TELEMETRY_TIMEOUT")
        if not raw:
    ...
    def _is_valid_endpoint(self, candidate: str) -> bool:
        ...
        if host in ("localhost", "127.0.0.1", "::1") and not self.allow_loopback:
            ...
            ## RUNTIME_STUDIO_TELEMETRY_ALLOW_LOOPBACK=1.
            return False
        return True
    ...
    def _resolve_data_directory() -> Path:
        ...
        return base / "runtime-studio-godot"
...
class TelemetryCollector:
    ...
    def __init__(self, config: TelemetryConfig | None = None) -> None:
        ...
        self._worker = threading.Thread(
            target=self._worker_loop, name="runtime-studio-godot-telemetry", daemon=True
        )
        self._worker.start()
    ...
    def shutdown(self) -> None:
        ...
        ## ``RUNTIME_STUDIO_TELEMETRY_TIMEOUT`` allowed to exceed
