"""Runtime Studio for Godot — production-grade Godot MCP server."""
...
def _resolve_version(package_file: str | Path) -> str:
    ...
    ## METADATA at install time, so `importlib.metadata.version("runtime-studio-godot")`
    ...
    ## live pyproject keeps `runtime_studio.__version__` and `session_list`'s
    ...
    if pyproject.is_file():
        try:
            ...
            pass
    try:
        return _pkg_version("runtime-studio-godot")
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Runtime Studio for Godot server")
    parser.add_argument(
        "--version",
        action="version",
        version=f"runtime-studio-godot {__version__}",
    )
    parser.add_argument(
        "--transport",
    ...
    from runtime_studio.tools.domains import parse_exclude_list
    ...
    from runtime_studio.runtime_info import install_pid_file

    install_pid_file(args.pid_file)

    if args.reload and args.transport in ("sse", "streamable-http"):
        from runtime_studio.asgi import run_with_reload
