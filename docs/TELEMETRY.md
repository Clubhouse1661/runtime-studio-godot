# Runtime Studio for Godot Telemetry

Runtime Studio for Godot includes anonymous, privacy-focused telemetry that helps us
...
`src/runtime_studio/telemetry.py` and `plugin/addons/runtime_studio/telemetry.gd`.

## Privacy first

- **Anonymous**: a randomly generated UUID per installation. No account,
  no email, no machine fingerprint beyond OS / Python version.
- **Hashed session ids**: Runtime Studio for Godot session ids include a project-directory
...
Allowlist (mirrored in `plugin/addons/runtime_studio/telemetry.gd` and
`src/runtime_studio/transport/websocket.py::_PLUGIN_EVENT_NAMES`):
...
Open the "Clients & Settings" popup from the Runtime Studio for Godot dock, go to the
...
runtime-studio-godot maintainers. The endpoint URL lives in
``TelemetryConfig.DEFAULT_ENDPOINT`` (`src/runtime_studio/telemetry.py`); see
...
export RUNTIME_STUDIO_TELEMETRY_ENDPOINT=https://telemetry.example.com/events

# Optional: customize request timeout (default 1.5 seconds):
export RUNTIME_STUDIO_TELEMETRY_TIMEOUT=2.5

# Local-sink smoke testing (loopback endpoints are otherwise rejected):
export RUNTIME_STUDIO_TELEMETRY_ALLOW_LOOPBACK=1
export RUNTIME_STUDIO_TELEMETRY_ENDPOINT=http://127.0.0.1:7777/
```

Only `http://` and `https://` schemes are accepted; localhost is rejected
unless `RUNTIME_STUDIO_TELEMETRY_ALLOW_LOOPBACK=1` is also set. An invalid
...
- **macOS**: `~/Library/Application Support/runtime-studio-godot/`
- **Linux**: `$XDG_DATA_HOME/runtime-studio-godot/` (default `~/.local/share/runtime-studio-godot/`)
- **Windows**: `%APPDATA%\runtime-studio-godot\`
...
- `src/runtime_studio/telemetry.py` — collector, decorators, FastMCP wrap helper.
- `src/runtime_studio/server.py` — calls `install_fastmcp_wraps(mcp)` once,
  before any tool registration; emits `STARTUP` on lifespan enter.
- `src/runtime_studio/sessions/registry.py` — emits `GODOT_CONNECTION` on
  `register` / `unregister`.
- `src/runtime_studio/transport/websocket.py` — routes `plugin_event` envelopes
  through the allowlist.
- `plugin/addons/runtime_studio/telemetry.gd` — plugin-side helper.
...
1. Add the name to the allowlist in
   `plugin/addons/runtime_studio/telemetry.gd` (`_ALLOWED_EVENTS`).
2. Mirror it in `src/runtime_studio/transport/websocket.py`
   (`_PLUGIN_EVENT_NAMES`).
