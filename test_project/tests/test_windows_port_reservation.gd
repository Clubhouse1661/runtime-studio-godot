...
func test_hint_empty_when_no_match() -> void:
	...
	assert_eq(McpWindowsPortReservation.hint_from_output(lines, 8000), "")


func test_hint_empty_for_empty_input() -> void:
	assert_eq(McpWindowsPortReservation.hint_from_output(PackedStringArray(), 8000), "")
...
func test_port_excluded_hint_interpolates_port() -> void:
	...
	assert_contains(McpWindowsPortReservation.port_excluded_hint(8001), "8001")
	assert_contains(McpWindowsPortReservation.port_excluded_hint(9500), "9500")
	assert_contains(McpWindowsPortReservation.port_excluded_hint(8001), "winnat")
...
func test_hint_from_output_agrees_with_proactive_hint() -> void:
	...
	assert_eq(
		McpWindowsPortReservation.hint_from_output(lines, 8123),
		McpWindowsPortReservation.port_excluded_hint(8123)
	)
