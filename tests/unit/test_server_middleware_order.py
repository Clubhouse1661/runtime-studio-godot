...
above the ``mcp.add_middleware(...)`` calls in ``src/runtime_studio/server.py``
...
It also lets the runtime_studio-vs-FastMCP-internal split be expressed by an
...
def test_runtime_studio_middleware_registered_in_documented_order() -> None:
    ...
    runtime_studio middleware classes. FastMCP itself adds internal middleware
    ...
    assert actual == EXPECTED_ORDER, (
        "runtime_studio middleware registration order drifted.\n"
        ...
        "mcp.add_middleware(...) calls in src/runtime_studio/server.py and "
