...
const ADDON_PATH_MARKER := "/addons/runtime_studio/"
...
func _log_error(
	...
	if _is_in_runtime_studio_addon(resolved.path):
		return
	if not message_res_path.is_empty() and _is_in_runtime_studio_addon(message_res_path):
		return
...
## Path-substring check works for both `res://addons/runtime_studio/foo.gd` and
## globalized absolute paths (`/Users/.../addons/runtime_studio/foo.gd`) that
## Godot can also report depending on where the error originated.
static func _is_in_runtime_studio_addon(path: String) -> bool:
	if path.begins_with("res://addons/runtime_studio/"):
		return true
...
static func _extract_user_res_path(message: String) -> String:
	...
	if path.is_empty() or _is_in_runtime_studio_addon(path):
		return ""
	return path
