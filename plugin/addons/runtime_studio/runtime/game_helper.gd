...
## Runtime Studio for Godot MCP — game-process helper.
##
## Registered as an autoload by plugin.gd when the Runtime Studio for Godot plugin is enabled.
...
const GAME_LOGGER_PATH := "res://addons/runtime_studio/runtime/game_logger.gd"
...
func _ready() -> void:
	...
	print("[runtime_studio game_helper] registered mcp capture (debugger active=%s, logger=%s)"
