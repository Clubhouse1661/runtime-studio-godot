...
static func remove(client: McpClient, _server_name: String) -> Dictionary:
	...
	## Subtables in the namespace (e.g. [mcp_servers.runtime-studio-godot.tools.session_list]
	## that codex users add to set per-tool approval_mode) must be removed
	## too. Leaving them behind keeps `mcp_servers.runtime-studio-godot` implicitly
	## defined, so a later configure that writes [mcp_servers."runtime-studio-godot"]
...
static func _all_headers(client: McpClient) -> Array[String]:
	...
	## so [mcp_servers.runtime-studio-godot] is a valid hand-written form of the same
	## logical key we emit as [mcp_servers."runtime-studio-godot"]. Match both during
...
## `[mcp_servers."runtime-studio-godot"` is a prefix of `[mcp_servers."runtime-studio-godot-dev"]`,
