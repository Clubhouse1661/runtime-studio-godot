...
##   - keeps server-launch discovery (.venv → uvx → system runtime-studio-godot)
...
const ManualCommand := preload("res://addons/runtime_studio/clients/_manual_command.gd")
const CliFinder := preload("res://addons/runtime_studio/clients/_cli_finder.gd")
const WindowsPortReservation := preload("res://addons/runtime_studio/utils/windows_port_reservation.gd")

const SERVER_NAME := "runtime-studio-godot"
...
## Editor Settings > Plugins > runtime_studio without touching code. See #146 for
## the Windows-reservation diagnostics this is the escape hatch for.
const DEFAULT_HTTP_PORT := 8000
const DEFAULT_WS_PORT := 9500
const STARTUP_TRACE_ENV := "RUNTIME_STUDIO_STARTUP_TRACE"
const MIN_PORT := 1024
const MAX_PORT := 65535
const SETTING_WS_PORT := "runtime_studio/ws_port"
const SETTING_STARTUP_TRACE := "runtime_studio/log_startup_timing"
...
## Read the `runtime_studio/excluded_domains` EditorSetting as a canonicalized
...
static func _verify_post_state(
	...
	var path_hint := "" if path.is_empty() else " Inspect %s and remove the runtime-studio-godot entry by hand if needed." % path
	return {
		"status": "error",
...
#   3. runtime-studio-godot CLI  — system-wide pip/pipx/uv install

static func get_plugin_version() -> String:
	var cfg := ConfigFile.new()
	if cfg.load("res://addons/runtime_studio/plugin.cfg") == OK:
...
## checkout (there's a .venv nearby but `addons/runtime_studio` is a plain copy —
## e.g. after unpacking a release zip into `test_project/`).
##
## Two ways to set it, resolved in priority order:
##   1. EditorSettings → `runtime_studio/mode_override` — UI dropdown in the dock,
##      persists per-editor-install. Wins over the env var so a UI action
##      always takes effect without relaunching the editor.
##   2. Env var `RUNTIME_STUDIO_MODE` — useful for CLI launches and CI.
const MODE_OVERRIDE_ENV := "RUNTIME_STUDIO_MODE"
const MODE_OVERRIDE_SETTING := "runtime_studio/mode_override"
...
## Data-safety check for self-install: is `res://addons/runtime_studio` a symbolic
...
static func addons_dir_is_symlink() -> bool:
	return _is_symlink(ProjectSettings.globalize_path("res://addons/runtime_studio"))
...
static func get_server_command(refresh: bool = false) -> Array[String]:
	...
	if mode_override() != "user":
		...
		if not venv_python.is_empty():
			...
			return [venv_python, "-m", "runtime_studio"]
	...
	if not uvx.is_empty():
		...
		print("MCP | using uvx (runtime-studio-godot==%s)%s" % [version, " [refresh]" if refresh else ""])
		var cmd: Array[String] = [uvx]
		if refresh:
			cmd.append("--refresh")
		cmd.append_array(["--from", "runtime-studio-godot==%s" % version, "runtime-studio-godot"])
		return cmd
	...
	push_warning("MCP | no server found — install uv or run: pip install runtime-studio-godot")
	return []
...
## monkey-patching `res://`. Only treats a `.venv/bin/python` as a runtime-studio-godot dev
## venv if a sibling `src/runtime_studio/` exists in the same parent dir — otherwise
## an unrelated user venv (e.g. `~/.venv` from a data-science side project)
## gets picked up and `python -m runtime_studio` fails with ModuleNotFoundError about
...
static func _find_venv_python_in(start_dir: String) -> String:
	...
	for i in 5:
		...
		if FileAccess.file_exists(venv_path) and DirAccess.dir_exists_absolute(dir.path_join("src/runtime_studio")):
			return venv_path
...
## Walk up from `start_dir` looking for a sibling `src/runtime_studio/` — returns
...
static func find_worktree_src_dir(start_dir: String) -> String:
	var dir := start_dir.rstrip("/")
	for i in 5:
		var candidate := dir.path_join("src/runtime_studio")
...
static func _find_system_install() -> String:
	...
	var output: Array = []
	if OS.execute(cmd, ["runtime-studio-godot"], output, true) == 0 and output.size() > 0:
