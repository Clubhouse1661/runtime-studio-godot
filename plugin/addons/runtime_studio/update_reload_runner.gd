...
const PENDING_SELF_UPDATE_TELEMETRY_KEY := "runtime_studio/pending_self_update_event"
...
## `addons/runtime_studio/` with rollback bookkeeping, scan the filesystem,
...
const PLUGIN_CFG_PATH := "res://addons/runtime_studio/plugin.cfg"
...
const ZIP_ADDON_PREFIX := "addons/runtime_studio/"
const TEMP_FILE_SUFFIX := ".runtime_studio_update_tmp"
...
func _read_update_manifest() -> bool:
	...
	for file_path in files:
		...
		## `addons/runtime_studio/`. Skip those before the safety check; the
...
func _handle_install_failure(status: int) -> void:
	...
	if status == InstallStatus.FAILED_MIXED:
		...
		push_error(
			...
			+ " restore the previous addons/runtime_studio/ contents. The plugin"
			+ " is left disabled. Inspect addons/runtime_studio/ for"
			+ " *.update_backup / *.runtime_studio_update_tmp files and restore"
			+ " manually before re-enabling the plugin."
		)
		print(
			"MCP | self-update aborted: addons/runtime_studio/ is in a mixed state;"
