...
## Scanner that detects whether `addons/runtime_studio/` is in a half-installed
...
const ADDON_DIR := "res://addons/runtime_studio/"
...
static func diagnose(dir: String = ADDON_DIR, force: bool = false) -> Dictionary:
	...
	if not backups.is_empty():
		...
		result = {
			...
			"message": (
				"Found .update_backup files in addons/runtime_studio/. This usually"
