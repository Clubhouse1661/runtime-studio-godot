...
const McpServerStateScript := preload("res://addons/runtime_studio/utils/mcp_server_state.gd")
const McpStartupPathScript := preload("res://addons/runtime_studio/utils/mcp_startup_path.gd")
const McpAdoptionLabelScript := preload("res://addons/runtime_studio/utils/mcp_adoption_label.gd")
const McpServerVersionCheckScript := preload("res://addons/runtime_studio/utils/server_version_check.gd")
...
func handle_server_version_verified(expected_version: String, version: String) -> void:
	_server_actual_name = "runtime-studio-godot"
	...
	if compatibility.get("compatible", false):
		...
		return
	var live := {"version": version, "status_code": 200, "name": "runtime-studio-godot"}
...
static func _incompatible_server_message(
	...
	## can tell *which* `src/runtime_studio/` is serving the port without
	...
	if not version.is_empty():
		if actual_ws_port > 0 and actual_ws_port != expected_ws_port:
			return (
				"Port %d is occupied by runtime-studio-godot server v%s using WS port %d%s; "
		...
		return (
			"Port %d is occupied by runtime-studio-godot server v%s%s; plugin expects v%s. "
	...
	if status_code > 0:
		return (
			...
			+ "plugin expects runtime-studio-godot v%s. Stop the other server or change both HTTP and WS ports."
		) % [port, status_code, expected_version]
	return (
		"Port %d is occupied by another process; plugin expects runtime-studio-godot v%s. "
...
static func _live_status_identifies_runtime_studio(live: Dictionary) -> bool:
	return str(live.get("name", "")) == "runtime-studio-godot"


static func _live_version_for_message(live: Dictionary) -> String:
	if live.has("name") and str(live.get("name", "")) != "runtime-studio-godot":
		return ""
...
static func _live_package_path_for_message(live: Dictionary) -> String:
	## Only trust the path when the live snapshot confirms a runtime-studio-godot
	...
	## that as "runtime-studio-godot loaded from …" in the incompatible banner.
	if live.has("name") and str(live.get("name", "")) != "runtime-studio-godot":
		return ""
...
## Sets RUNTIME_STUDIO_DISABLE_TELEMETRY in the process environment for the
## upcoming OS.create_process call if: (a) neither RUNTIME_STUDIO_DISABLE_TELEMETRY
## nor DISABLE_TELEMETRY is already set, and (b) the EditorSettings key
## "runtime_studio/telemetry_enabled" is set to false. Returns true if the var was
...
func _inject_telemetry_env() -> bool:
	...
	if OS.has_environment("RUNTIME_STUDIO_DISABLE_TELEMETRY") or OS.has_environment("DISABLE_TELEMETRY"):
		return false
	if not McpSettings.telemetry_enabled():
		OS.set_environment("RUNTIME_STUDIO_DISABLE_TELEMETRY", "true")
		return true
	return false
...
func start_server() -> void:
	...
	## against a worktree whose `src/runtime_studio/__version__` differs from the
...
func stop_server() -> void:
	...
	if real_pid > 0 and (
		...
		or _host._pid_cmdline_is_runtime_studio_for_proof(real_pid)
	):
	...
	for pid in listener_pids:
		...
		elif _host._pid_cmdline_is_runtime_studio_for_proof(listener_pid):
			candidates.append(listener_pid)
