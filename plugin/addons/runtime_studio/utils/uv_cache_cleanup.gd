...
## a running runtime-studio-godot server, uv builds an ephemeral venv under
...
##
## If the runtime-studio-godot server's own Python child has that same `.pyd` mapped via
## `LoadLibrary` (it does — runtime-studio-godot imports pydantic), the file is locked
