...
const RELEASES_URL := (
	"https://api.github.com/repos/Clubhouse1661/runtime-studio-godot/releases/latest"
)
const RELEASES_PAGE := "https://github.com/Clubhouse1661/runtime-studio-godot/releases/latest"
const UPDATE_TEMP_DIR := "user://runtime_studio_update/"
const UPDATE_TEMP_ZIP := "user://runtime_studio_update/update.zip"
...
## `addons/runtime_studio/` is a symlink into canonical `plugin/` source there,
## and an extract would clobber tracked files (#116). `is_dev_checkout()`
## honours the mode override (dock dropdown > RUNTIME_STUDIO_MODE env), so
...
##   download_url: String            ## matching `runtime-studio-godot-plugin.zip` asset URL
##
...
static func parse_releases_response(
	...
	for asset in assets:
		...
		if String(asset_dict.get("name", "")) == "runtime-studio-godot-plugin.zip":
...
func _install_zip_inline(version: Dictionary) -> void:
	...
	for file_path in files:
		if not file_path.begins_with("addons/runtime_studio/"):
			continue
