...
## Shared EditorSettings key constants for the runtime_studio/* namespace.
##
...
const SETTING_HTTP_PORT := "runtime_studio/http_port"
## Comma-separated list of tool domains excluded from the server at spawn time.
const SETTING_EXCLUDED_DOMAINS := "runtime_studio/excluded_domains"
const SETTING_TELEMETRY_ENABLED := "runtime_studio/telemetry_enabled"
...
##   1. RUNTIME_STUDIO_DISABLE_TELEMETRY / DISABLE_TELEMETRY env vars
##   2. The runtime_studio/telemetry_enabled EditorSetting written by the dock UI
## Defaults to true when neither source has set a preference.
static func telemetry_enabled() -> bool:
	if env_truthy("RUNTIME_STUDIO_DISABLE_TELEMETRY") or env_truthy("DISABLE_TELEMETRY"):
		return false
