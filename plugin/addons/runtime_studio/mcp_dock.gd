...
## res://addons/runtime_studio/dock_panels/:
...
const ClientRefreshStateScript := preload("res://addons/runtime_studio/utils/mcp_client_refresh_state.gd")
const Telemetry := preload("res://addons/runtime_studio/telemetry.gd")
const UpdateManagerScript := preload("res://addons/runtime_studio/utils/update_manager.gd")
const UpdateMixedStateScript := preload("res://addons/runtime_studio/utils/update_mixed_state.gd")
const Client := preload("res://addons/runtime_studio/clients/_base.gd")
const ClientConfigurator := preload("res://addons/runtime_studio/client_configurator.gd")
const ClientRegistry := preload("res://addons/runtime_studio/clients/_registry.gd")
const JsonStrategy := preload("res://addons/runtime_studio/clients/_json_strategy.gd")
const TomlStrategy := preload("res://addons/runtime_studio/clients/_toml_strategy.gd")
const CliStrategy := preload("res://addons/runtime_studio/clients/_cli_strategy.gd")
const ToolCatalog := preload("res://addons/runtime_studio/tool_catalog.gd")
...
const DEV_MODE_SETTING := "runtime_studio/dev_mode"
...
# the user changes `runtime_studio/http_port`). Refreshes are stale-while-refreshing:
...
# Mixed-state banner — surfaces when `addons/runtime_studio/` contains
...
func _build_ui() -> void:
	...
	_crash_restart_btn.tooltip_text = "Stop the old server on this port and start the bundled runtime-studio-godot server"
...
## port alone is a valid recovery. Incompatible runtime-studio-godot servers commonly
	...
	match state:
		...
		ServerStateScript.INCOMPATIBLE:
			...
			if bool(server_status.get("can_recover_incompatible", false)):
				...
				if not message.is_empty():
					return "%s Click Restart Server below to replace it with runtime-studio-godot v%s." % [message, expected]
				return "Port %d is occupied by an older runtime-studio-godot server. Click Restart Server below to replace it with runtime-studio-godot v%s." % [port, expected]
			if not message.is_empty():
				return message
		...
		ServerStateScript.CRASHED:
			...
			if ClientConfigurator.get_server_launch_mode() == "uvx":
				...
				return "The server exited before the WebSocket handshake, even after a `uvx --refresh` retry. If this is a brand-new release, PyPI's index may still be propagating (~10 min). Wait a moment and click Reload Plugin to retry, or check Godot's output log for Python's traceback. Target: runtime-studio-godot==%s." % version
		...
		ServerStateScript.NO_COMMAND:
			return "No runtime-studio-godot server found. Install `uv` via the Setup panel above, or run `pip install runtime-studio-godot`."
		_:
			return ""
...
func _build_mixed_state_banner() -> void:
	...
	_mixed_state_rescan_btn.tooltip_text = (
		"Scan addons/runtime_studio/ for *.update_backup files again."
...
## pinned without polluting the real `addons/runtime_studio/` tree with backup
...
## Returns true if RUNTIME_STUDIO_DISABLE_TELEMETRY or DISABLE_TELEMETRY is set
...
func _is_telemetry_disabled_via_env() -> Variant:
	if not (OS.has_environment("RUNTIME_STUDIO_DISABLE_TELEMETRY") or OS.has_environment("DISABLE_TELEMETRY")):
		return null
	return McpSettings.env_truthy("RUNTIME_STUDIO_DISABLE_TELEMETRY") or McpSettings.env_truthy("DISABLE_TELEMETRY")
...
func _load_telemetry_setting() -> void:
	...
	if env_disabled != null:
		...
		_telemetry_toggle.tooltip_text = (
			...
			+ "(RUNTIME_STUDIO_DISABLE_TELEMETRY / DISABLE_TELEMETRY)."
		)
	else:
...
func _do_plugin_reload() -> void:
	EditorInterface.set_plugin_enabled("res://addons/runtime_studio/plugin.cfg", false)
	EditorInterface.set_plugin_enabled("res://addons/runtime_studio/plugin.cfg", true)
...
func _refresh_server_version_label(server_status: Dictionary = {}) -> void:
	...
	if _server_restart_in_progress:
		...
		text = "checking live version (expected runtime-studio-godot == %s)" % expected_ver
		color = COLOR_MUTED
	elif server_ver == expected_ver:
		text = "runtime-studio-godot == %s" % server_ver
		color = Color.GREEN
	else:
		text = "runtime-studio-godot == %s  (expected %s)" % [server_ver, expected_ver]
...
func _resolve_plugin_symlink_target() -> String:
	var addons_path := ProjectSettings.globalize_path("res://addons/runtime_studio")
