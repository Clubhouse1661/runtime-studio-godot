...
const GAME_HELPER_AUTOLOAD_PATH := "res://addons/runtime_studio/runtime/game_helper.gd"
...
const EDITOR_LOGGER_PATH := "res://addons/runtime_studio/runtime/editor_logger.gd"
...
const MANAGED_SERVER_PID_SETTING := "runtime_studio/managed_server_pid"
const MANAGED_SERVER_VERSION_SETTING := "runtime_studio/managed_server_version"
const MANAGED_SERVER_WS_PORT_SETTING := "runtime_studio/managed_server_ws_port"
const UPDATE_RELOAD_RUNNER_SCRIPT := preload("res://addons/runtime_studio/update_reload_runner.gd")
...
const UvCacheCleanup := preload("res://addons/runtime_studio/utils/uv_cache_cleanup.gd")
...
## lifecycle manager.
const ServerLifecycleManager := preload("res://addons/runtime_studio/utils/server_lifecycle.gd")
const PortResolver := preload("res://addons/runtime_studio/utils/port_resolver.gd")
const ServerStateScript := preload("res://addons/runtime_studio/utils/mcp_server_state.gd")
const StartupPathScript := preload("res://addons/runtime_studio/utils/mcp_startup_path.gd")
...
const Connection := preload("res://addons/runtime_studio/connection.gd")
const Dispatcher := preload("res://addons/runtime_studio/dispatcher.gd")
const Telemetry := preload("res://addons/runtime_studio/telemetry.gd")
const LogBuffer := preload("res://addons/runtime_studio/utils/log_buffer.gd")
const GameLogBuffer := preload("res://addons/runtime_studio/utils/game_log_buffer.gd")
const EditorLogBuffer := preload("res://addons/runtime_studio/utils/editor_log_buffer.gd")
const Dock := preload("res://addons/runtime_studio/mcp_dock.gd")
const DebuggerPlugin := preload("res://addons/runtime_studio/debugger/mcp_debugger_plugin.gd")
const ClientConfigurator := preload("res://addons/runtime_studio/client_configurator.gd")
const WindowsPortReservation := preload("res://addons/runtime_studio/utils/windows_port_reservation.gd")
...
const SceneHandler := preload("res://addons/runtime_studio/handlers/scene_handler.gd")
...
const FilesystemHandler := preload("res://addons/runtime_studio/handlers/filesystem_handler.gd")
const SignalHandler := preload("res://addons/runtime_studio/handlers/signal_handler.gd")
const AutoloadHandler := preload("res://addons/runtime_studio/handlers/autoload_handler.gd")
const InputHandler := preload("res://addons/runtime_studio/handlers/input_handler.gd")
const TestHandler := preload("res://addons/runtime_studio/handlers/test_handler.gd")
...
const ThemeHandler := preload("res://addons/runtime_studio/handlers/theme_handler.gd")
const AnimationHandler := preload("res://addons/runtime_studio/handlers/animation_handler.gd")
const MaterialHandler := preload("res://addons/runtime_studio/handlers/material_handler.gd")
...
const PhysicsShapeHandler := preload("res://addons/runtime_studio/handlers/physics_shape_handler.gd")
const EnvironmentHandler := preload("res://addons/runtime_studio/handlers/environment_handler.gd")
const TextureHandler := preload("res://addons/runtime_studio/handlers/texture_handler.gd")
...
const SERVER_STATUS_PATH := "/runtime-studio-godot/status"
...
func _enter_tree() -> void:
	...
	# Dock panel
	_dock = Dock.new()
	_dock.name = "Runtime Studio for Godot"
...
static func _mcp_disabled_for_headless_launch() -> bool:
	return _mcp_disabled_for_headless(
		...
		OS.get_environment("RUNTIME_STUDIO_ALLOW_HEADLESS")
	)
...
## tests assert against `RuntimeStudioPlugin._incompatible_server_message`.
...
static func _live_status_identifies_runtime_studio(live: Dictionary) -> bool:
	return ServerLifecycleManager._live_status_identifies_runtime_studio(live)


func _verified_status_version(live: Dictionary) -> String:
	if not ServerLifecycleManager._live_status_identifies_runtime_studio(live):
		return ""
...
func _evaluate_strong_port_occupant_proof(port: int, live: Dictionary = {}) -> Dictionary:
	...
	if (
		_live_status_identifies_runtime_studio(current_live)
...
func _evaluate_recovery_port_occupant_proof(port: int, live: Dictionary = {}) -> Dictionary:
	...
	var current_live: Dictionary = live if not live.is_empty() else _probe_live_server_status_for_port(port)
	if _live_status_identifies_runtime_studio(current_live):
...
func _legacy_pidfile_kill_targets(_port: int, listener_pids: Array[int]) -> Array[int]:
	...
	if not _pid_alive_for_proof(pidfile_pid) or not _pid_cmdline_is_runtime_studio_for_proof(pidfile_pid):
		return targets
	...
	for pid in listener_pids:
		...
		if pid == pidfile_pid or _pid_cmdline_is_runtime_studio_for_proof(pid):
			targets.append(pid)
...
func _pid_cmdline_is_runtime_studio_for_proof(pid: int) -> bool:
	return _pid_cmdline_is_runtime_studio(pid)
...
## `runtime-studio-godot` brand AND a server flag (`--pid-file` / `--transport`). Used by
...
func _pid_cmdline_is_runtime_studio(pid: int) -> bool:
	...
	## parent reloader carries the runtime_studio brand. Bound the walk so a
	...
	for _i in range(5):
		...
		if _commandline_is_runtime_studio_server(cmd):
			return true
...
static func _commandline_is_runtime_studio_server(cmd: String) -> bool:
	if cmd.is_empty():
		return false
	var lower := cmd.to_lower()
	## The server is invoked with `--pid-file <user>/runtime_studio_server.pid`,
	## so the path itself contains "runtime_studio". A naive substring brand
	...
	var has_brand := brand_search.find("runtime-studio-godot") >= 0 or brand_search.find("runtime_studio") >= 0
...
func start_dev_server() -> void:
	...
	## `src/runtime_studio/` (root repo or a git worktree), prepend that `src/` to
	## PYTHONPATH so `import runtime_studio` and uvicorn's `reload_dirs` both pick
	...
	get_tree().create_timer(0.5).timeout.connect(func():
		...
		if injected_telemetry:
			OS.unset_environment("RUNTIME_STUDIO_DISABLE_TELEMETRY")
...
func stop_dev_server() -> void:
	...
	for pid in _find_all_pids_on_port(port):
		...
		if _pid_cmdline_is_runtime_studio(candidate):
...
func is_dev_server_running() -> bool:
	...
	for pid in _find_all_pids_on_port(ClientConfigurator.http_port()):
		if _pid_cmdline_is_runtime_studio(int(pid)):
			return true
	return false
